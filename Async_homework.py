...
async def running_program():
    async with ClientSession() as session:
        result = await asyncio.gather(weatherstack(API[0], session), metaweather(API[1], session), oceandrivers(API[2], session))
        print(f'Median temperature is: {sum(result)/len(result)}')
